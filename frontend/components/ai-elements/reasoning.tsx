...
export const ReasoningTrigger = memo(
  ({
    ...
    const { t } = useLanguage();
    const reasoningDict = t.panel.chat.reasoning;

    const resolveThinkingMessage = () => {
      // If a custom getThinkingMessage prop was provided, let it win untouched.
      if (getThinkingMessage) {
        return getThinkingMessage(isStreaming, duration);
      }

      // Otherwise use the locale-aware default.
      if (isStreaming || duration === 0) {
        return (
          <Shimmer className="font-medium" duration={1}>
            {reasoningDict.thinking}
          </Shimmer>
        );
      }
      if (duration === undefined) {
        return <p>{reasoningDict.fewSeconds}</p>;
      }
      return (
        <p>
          {reasoningDict.forSeconds.replace("{seconds}", String(duration))}
        </p>
      );
    };
