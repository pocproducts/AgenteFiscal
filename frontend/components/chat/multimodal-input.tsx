...
function PureMultimodalInput({
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      toast.error(t.panel.chat.upload.failedToUpload);
    }
  }, []);
  ...
  const handleFileChange = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        toast.error(t.panel.chat.upload.failedFiles);
      } finally {
        setUploadQueue([]);
      }
    },
  ...
  const handlePaste = useCallback(
    async (event: ClipboardEvent) => {
      ...
      try {
        ...
        toast.error(t.panel.chat.upload.failedPasted);
      } finally {
        setUploadQueue([]);
      }
    },
        ...
        {profiles.length > 0 ? (
          <Select onValueChange={setActiveProfileId} value={activeProfileId}>
            <SelectTrigger className="h-7 border-none bg-transparent px-2 text-xs text-muted-foreground hover:text-foreground shadow-none focus:ring-0 gap-1 rounded-lg">
              <SelectValue placeholder={t.panel.chat.profile.selectPlaceholder} />
            </SelectTrigger>
...
function PureModelSelectorCompact({
  ...
  return (
    <ModelSelector onOpenChange={setOpen} open={open}>
      ...
      <ModelSelectorContent>
        <ModelSelectorInput placeholder={t.panel.chat.modelPlaceholder} />
        <ModelSelectorList>
          {(() => {
