...
Implements :class:`fiscal_agent.ports.api_keys.ApiKeyPort` (auth resolution)
and :class:`fiscal_agent.ports.api_keys.ApiKeyRepository` (admin CRUD) against
the ORM models in ``fiscal_agent.db.models.core``. Redis is no longer the
...
from fiscal_agent.db.auth import get_active_plan
...
from fiscal_agent.domain.models import ApiKey, App, Developer
from fiscal_agent.ports.api_keys import ApiKeyContext, ApiKeyStoreUnavailableError
...
class PostgresApiKeyPort:
	...
	``fiscal_agent.db.session``). Sessions are created per call, so the port is
