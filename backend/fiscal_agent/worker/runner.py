	...
	async def process_run(self, report_run_id: UUID) -> None:
		...
		async with self._session_factory() as session:
			...
			try:
				...
				cliente = ClientConfig(cuit=run.cuit)
...
def _load_smtp_config() -> Optional[AppConfig]:
	"""Load SMTP config from ``clients.yaml`` if present (best-effort)."""
	import yaml
	...
	if not raw or not raw.get('smtp'):
		return None
