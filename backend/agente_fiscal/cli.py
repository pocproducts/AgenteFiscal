"""Agente Fiscal CLI — pipeline orchestration.

Usage:
        python -m agente_fiscal run --config clients.yaml
        python -m agente_fiscal validate [config_path]
        python -m agente_fiscal generate-template [output_path]
"""
...
from agente_fiscal.adapters.arca_ws import consultar_cuit, get_ta
...
app = typer.Typer(
	name='agente-fiscal',
	help='Agente Fiscal — calendario de vencimientos ARCA',
)
...
def report(
	...
	Uso::
	    ...
	    uv run python -m agente_fiscal report
	    uv run python -m agente_fiscal report --headed
	"""
	load_dotenv()
	now = datetime.now()
	mes = now.month
...
def main() -> None:
	"""Entry point for ``python -m agente_fiscal``."""
	app()
