"""SMTP email sender for fiscal calendar PDF delivery — legacy CLI path.
...
Only used by the CLI's ``clients.yaml`` batch mode (``cli.py``). The live
API/worker paths default to ``adapters.resend_email.ResendEmailSender``
instead (see ``pipeline/service.py::_email_sender_obj``); both implement the
same ``agente_fiscal.ports.email.EmailSenderPort`` contract.
"""
...
from agente_fiscal.domain.models import ClientConfig, SmtpConfig
...
class EmailSender:
	...
	def _build_message(
		...
		msg['Subject'] = build_subject(cliente.nombre, mes, anio)
		msg['Reply-To'] = self.smtp.from_addr

		# Body
		body = build_body(cliente.nombre, mes, anio)
