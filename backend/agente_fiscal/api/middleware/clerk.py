...
(``agente_fiscal.db.auth``). Redis is used ONLY for the JWKS cache
...
#: JWKS cache TTL (seconds) — mirrors the Redis ``jwks:clerk`` TTL.
_JWKS_TTL = 3600
...
class ClerkJWTExtractor:
	...
	def __init__(self, session_factory: async_sessionmaker | None = None):
		...
		# In-memory JWKS fallback (same 3600s TTL) so Clerk auth keeps
		# working when Redis is unavailable; Redis is used when present.
		self._memory_jwks: dict | None = None
		self._memory_jwks_expires_at: float = 0.0

	def _set_memory_jwks(self, jwks: dict) -> None:
		"""Store JWKS in the process-local fallback cache with the shared TTL."""
		self._memory_jwks = jwks
		self._memory_jwks_expires_at = time.time() + _JWKS_TTL

	def _clear_memory_jwks(self) -> None:
		"""Drop the in-memory JWKS cache (used on key-rotation refetch)."""
		self._memory_jwks = None
		self._memory_jwks_expires_at = 0.0

	async def _get_jwks(self, redis: Redis | None) -> dict:
		"""Fetch Clerk JWKS, cached in memory and Redis with 3600s TTL.

		``redis`` may be ``None`` (degraded boot): the in-memory cache covers
		the same TTL and Redis is only consulted when available. Any Redis
		failure falls back to the memory cache too.
		...
		"""
		now = time.time()
		if self._memory_jwks is not None and self._memory_jwks_expires_at > now:
			return self._memory_jwks
		...
		if redis is not None:
			try:
				...
				if cached:
					...
					self._set_memory_jwks(jwks)
					return jwks
			except Exception:
				logger.warning('Redis JWKS cache no disponible — usando cache en memoria')

		settings = get_settings()
		domain = settings.clerk_domain
		if not domain:
			logger.error('CLERK_DOMAIN no está configurado')
			self._clear_memory_jwks()
			return {'keys': []}
		...
		if redis is not None:
			try:
				await redis.setex('jwks:clerk', _JWKS_TTL, json.dumps(jwks))
			except Exception:
				logger.warning('Redis JWKS cache write falló — cache en memoria solamente')
		return jwks

	async def verify_jwt(self, token: str, redis: Redis | None) -> dict | None:
		"""Validate a Clerk JWT and return its decoded payload.

		``redis`` may be ``None`` — the JWKS cache then degrades to memory.
			...
			if not jwk:
				...
				if redis is not None:
					try:
						...
						logger.warning('Redis JWKS delete falló — cache en memoria ya limpiada')
				jwks = await self._get_jwks(redis)
				keys = jwks.get('keys', [])
				jwk = next((k for k in keys if k.get('kid') == kid), None)
	...
	async def handle(self, token: str, request: Request, redis: Redis | None) -> bool:
