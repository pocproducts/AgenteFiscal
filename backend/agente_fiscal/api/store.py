"""Redis-backed store — best-effort conversation cache (cutover Phase 5).

Business data (API keys, apps, plans, developers, tenant admin CRUD, clients)
all left Redis in the Phase 5 cutover; their source of truth is now Postgres
behind the hexagonal ports (see ``adapters/db_api_keys.py``,
``adapters/db_clients.py``). What remains here is ``RedisStore`` — a
best-effort conversation cache (the frontend does not consume it yet; kept
intentionally) and its Redis key schema for conversations.
