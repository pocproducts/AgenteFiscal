...
from agente_fiscal.api.deps import get_memory
from agente_fiscal.api.routes.health import _check_composio, _check_engram, _check_redis, _check_ta
...
#: True once we've logged the Redis-down cache warning (per process).
_logged_no_redis_cache = False


def _note_redis_degraded() -> None:
	"""Log a single warning per process about Redis being unavailable."""
	global _logged_no_redis_cache
	if not _logged_no_redis_cache:
		_logged_no_redis_cache = True
		logger.warning('Redis no disponible — monitor operando sin cache')
...
async def _cache_get(request: Request, key: str) -> str | None:
	"""Read from the Redis cache, return None if missing/error/unavailable."""
	if getattr(request.app.state, 'redis', None) is None:
		_note_redis_degraded()
		return None
	try:
...
async def _cache_set(request: Request, key: str, value: str, ttl: int) -> None:
	"""Write to the Redis cache with TTL; no-op when Redis is unavailable."""
	if getattr(request.app.state, 'redis', None) is None:
		_note_redis_degraded()
		return
	try:
...
async def get_system_metrics(
	...
	try:
		search_results = memory._engram_get(  # noqa: SLF001
			f'/search?q=pipeline_run&project=agente-fiscal&type=pipeline_run&limit=500'
		)
		observations = (
			search_results
	...
	try:
		errors_results = memory._engram_get(  # noqa: SLF001
			f'/search?q=error&project=agente-fiscal&type=error&limit=20'
		)
		error_obs = (
			errors_results
...
async def get_system_activity(
	...
	try:
		search_results = memory._engram_get(  # noqa: SLF001
			f'/search?q=pipeline_run error&project=agente-fiscal&limit={limit + offset}'
		)
		observations = (
			search_results
			if isinstance(search_results, list)
...
async def get_system_errors(
	...
	try:
		search_results = memory._engram_get(  # noqa: SLF001
			f'/search?q=error&project=agente-fiscal&type=error&limit=200'
		)
		observations = (
			search_results
			if isinstance(search_results, list)
