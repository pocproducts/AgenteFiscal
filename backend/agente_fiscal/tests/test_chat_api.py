"""Integration tests for POST /v1/chat/message — agente_fiscal/api/routes/chat.py.
...
def app() -> FastAPI:
	...
	app = FastAPI()
	from agente_fiscal.api.routes.chat import router
...
def _mock_handler(intent: Intent, data: dict):
	...
	return patch('agente_fiscal.api.routes.chat._dispatch', return_value=data)
...
def test_error_backend(client: TestClient) -> None:
	...
	with patch('agente_fiscal.api.routes.chat._dispatch', side_effect=ValueError('Error interno')):
		resp = client.post(
			'/v1/chat/message',
			json={'message': 'consulta CUIT 20324837796'},
		)
