...
from agente_fiscal.adapters.memory import FiscalMemoryClient, MemoryConfig
...
class TestWriteMethods:
	...
	def _assert_observation_posted(
		...
		assert body['project'] == 'agente-fiscal'
	...
	def test_save_padron_result(self, client: FiscalMemoryClient) -> None:
		...
		with patch('agente_fiscal.adapters.memory.client.requests.post') as mock_post:
...
class TestReadMethods:
	...
	def _assert_search_url(self, url: str, cuit: str, obs_type: str | None = None) -> None:
		...
		assert cuit in url
		assert 'project=agente-fiscal' in url
	...
	def test_get_padron_history(self, client: FiscalMemoryClient) -> None:
		...
		with patch('agente_fiscal.adapters.memory.client.requests.get') as mock_get:
...
class TestBestEffort:
	...
	def test_write_silent_on_connection_error(self, client: FiscalMemoryClient) -> None:
		...
		with patch('agente_fiscal.adapters.memory.client.requests.post', side_effect=ConnectionError('No route to host')):
			# Must not raise
	...
	def test_read_returns_default_on_connection_error(self, client: FiscalMemoryClient) -> None:
		...
		with patch('agente_fiscal.adapters.memory.client.requests.get', side_effect=ConnectionError('No route to host')):
	...
	def test_requests_timeout(self, client: FiscalMemoryClient) -> None:
		...
		with patch('agente_fiscal.adapters.memory.client.requests.post', side_effect=requests.Timeout('timed out')):
...
class TestRedisHasSpace:
	...
	def test_cache_set_skips_when_full(self, client: FiscalMemoryClient) -> None:
		...
		with (
			...
			patch('agente_fiscal.adapters.memory.client.requests.get') as mock_get,
		):
