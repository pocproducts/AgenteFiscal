"""Tests for the in-process report worker (``agente_fiscal.worker.runner``).
...
async def test_queued_run_transitions_to_done_with_summary() -> None:
	...
	with (
		patch('agente_fiscal.worker.runner.PipelineService', FakePipelineService),
		patch('agente_fiscal.worker.runner.get_ta', return_value=('token', 'sign')),
	):
...
async def test_ta_unavailable_marks_failed() -> None:
	...
	with (
		patch('agente_fiscal.worker.runner.PipelineService', FakePipelineService),
		patch('agente_fiscal.worker.runner.get_ta', return_value=(None, None)),
	):
		await runner.process_run(run.id)
