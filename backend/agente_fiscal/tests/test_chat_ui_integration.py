...
class TestHistoryPrepend:
	...
	def test_history_prepend_adds_context(self) -> None:
		...
		from agente_fiscal.domain.intent_router import detect
...
class TestAutoPersist:
	...
	def test_sync_auto_persists(self, client: TestClient, app_with_store: FastAPI) -> None:
		...
		with patch('agente_fiscal.api.routes.chat._handle_taxpayer', return_value={'nombre': 'Test'}):
			resp = client.post(
				'/v1/chat/message',
	...
	def test_sse_auto_persists_before_complete(self, client: TestClient, app_with_store: FastAPI) -> None:
		...
		with patch('agente_fiscal.api.routes.chat._handle_reporte_with_echo', return_value={'calendario': {}}):
			resp = client.post(
